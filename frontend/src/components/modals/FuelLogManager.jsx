...
import { Fuel, Trash2, Plus, FileText, Loader2, Save, Pencil } from 'lucide-react';
import { toast } from 'react-toastify';

// --- FRONTEND SCHEMA ---
// Note: validation for file is different on frontend (FileList object) vs backend (string URL)
const fuelFormSchema = yup.object({
    amount: yup
        .number()
    ...
    invoiceSerial: yup
        ...
        .max(20, 'Too long'),
    // Optional file validation (max 5MB, image type) could be added here if needed
});
...
export default function FuelLogManager({ tripId, isReadOnly }) {
    ...
    const FuelForm = ({ defaultValues, onSave, onCancel, isLoading }) => {
        // Apply yupResolver here
        const { register, handleSubmit, formState: { errors } } = useForm({ 
            resolver: yupResolver(fuelFormSchema),
            defaultValues 
        });
        ...
        return (
            <form onSubmit={handleSubmit(onSubmit)} className="bg-surface border border-border rounded-lg p-3 space-y-3 animate-in fade-in">
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input 
                            {...register("amount")} 
                            type="number" 
                            className={`w-full px-2 py-1.5 text-sm border rounded bg-surface outline-none focus:ring-1 focus:ring-primary ${errors.amount ? 'border-red-500 focus:ring-red-500' : 'border-border'}`} 
                            placeholder="0" 
                        />
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            type="text" 
                            className={`w-full px-2 py-1.5 text-sm border rounded bg-surface outline-none focus:ring-1 focus:ring-primary ${errors.invoiceSerial ? 'border-red-500 focus:ring-red-500' : 'border-border'}`} 
                            placeholder="INV-..." 
                        />
                        {errors.invoiceSerial && <p className="text-red-500 text-[10px] mt-0.5">{errors.invoiceSerial.message}</p>}
                    </div>
                </div>
                <div>
                    ...
                    <input 
                        ...
                        type="file" 
                        accept="image/*,.pdf" 
                        className="text-xs text-muted-foreground file:mr-2 file:py-1 file:px-2 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20" 
                    />
                </div>
                <div className="flex justify-end gap-2 pt-1">
                    ...
                    <button type="submit" disabled={isLoading} className="px-3 py-1 text-xs rounded bg-primary text-text font-medium flex items-center gap-1 disabled:opacity-50">
    ...
    const handleCreate = async (formData) => {
        ...
        try {
            ...
            setIsAdding(false);
        } catch (err) { toast.error(err.response?.data?.message || "Error adding entry"); }
    };
    ...
    const handleUpdate = async (formData) => {
        try {
            ...
            setEditingId(null);
        } catch (err) { toast.error(err.response?.data?.message || "Error updating entry"); }
    };
                            ...
                            {editingId === entry._id ? (
                                ...
                                <div className="flex items-center justify-between p-3 bg-surface border border-border rounded-lg group hover:border-primary/30 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold text-text">{entry.amount} Liters</p>
                                    ...
                                    <div className="flex items-center gap-1">
                                        {entry.invoiceFile && (
                                            <a href={`http://localhost:5000/${entry.invoiceFile}`} target="_blank" rel="noreferrer" className="p-1.5 text-blue-500 hover:bg-blue-500/10 rounded transition-colors" title="View Invoice">
                                                <FileText size={14} />
                                            </a>
                                        )}
                                        {!isReadOnly && (
                                            <>
                                                <button onClick={() => setEditingId(entry._id)} className="p-1.5 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded transition-colors" title="Edit">
                                                    <Pencil size={14} />
                                                </button>
                                                <button onClick={() => { if(confirm('Delete this entry?')) deleteFuelEntry.mutate(entry._id) }} className="p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 rounded transition-colors" title="Delete">
                                                    <Trash2 size={14} />
                                                </button>
                                            </>
                                        )}
